---
title: What is MLX
description: Learn about the MLX inference runtime in LM Studio
index: 3
---
...
LM Studio integrates both mlx-lm and mlx-vlm into its MLX runtime backend. Because MLX is built by Apple for Apple hardware, it is a Mac-only runtime and is not available on other platforms.
...
Note that the MLX runtime only supports models in MLX format, and the llama.cpp runtime only supports models in GGUF format. 
...
Models stored locally in other formats can be converted to MLX format to use with the MLX runtime. See our guide on [converting models to MLX format](../modelyaml/convert-to-mlx).
